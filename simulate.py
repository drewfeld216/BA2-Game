from models import Session, create_db, Topic, Author, AuthorTopic, Event, EventTopic, Article, User, UserTopic, UserAuthor
...
def generate_pvs(start, end):
    with Session() as session:
        for day in tqdm(range(start, end)):
            ...
            events_today = session.query(Event) \
            ...
            if (len(events_today)):
                ...
                longtail = day
            articles_today = session.query(Article) \
...
def seed():
    ...
    with Session() as session:
        ...
        for topic_name, prob in generate_topics():
            t = Topic(name=topic_name, prob=prob)
            topics.append(t)
            session.add(t)
        session.commit()
        ...
        for author_name, quality, popularity, topic_probs in generate_authors():
            a = Author(name=author_name, quality=quality, popularity=popularity)
